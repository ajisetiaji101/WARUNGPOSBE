...
public class JwtUtil {
    ...
    private final String secret_key = "vivo21";
    ...
    public String createToken(AuthUserResponse userResponse){
        ...
        claims.put("level", userResponse.getLevel());
        claims.put("warung_pos_identity", userResponse.getWarung_pos_identity());

        Date tokenCreateTime = new Date();
        Date tokenValidity = new Date(tokenCreateTime.getTime() + TimeUnit.MINUTES.toMillis(accessTokenValidity));

        return Jwts.builder().setClaims(claims).setExpiration(tokenValidity).signWith(SignatureAlgorithm.HS256, secret_key).compact();
    }
